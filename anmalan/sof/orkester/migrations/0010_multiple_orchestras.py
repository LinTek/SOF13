...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Member.orchestra'
        db.delete_column('orkester_member', 'orchestra_id')

        # Adding M2M table for field orchestra on 'Member'
        db.create_table('orkester_member_orchestra', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('member', models.ForeignKey(orm['orkester.member'], null=False)),
            ('orchestra', models.ForeignKey(orm['orkester.orchestra'], null=False))
        ))
        db.create_unique('orkester_member_orchestra', ['member_id', 'orchestra_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Member.orchestra'
        raise RuntimeError("Cannot reverse this migration. 'Member.orchestra' and its values cannot be restored.")
        # Removing M2M table for field orchestra on 'Member'
        db.delete_table('orkester_member_orchestra')
    ...
    models = {
        'orkester.member': {
            ...
            'orchestra': ('django.db.models.fields.related.ManyToManyField', [], {'to': "orm['orkester.Orchestra']", 'symmetrical': 'False'}),
